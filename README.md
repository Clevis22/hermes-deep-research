...
One query, routed across **101 configured sources in 11 lanes** in parallel,
merged, deduplicated and grouped — with per-source coverage accounting, so a
source that failed is reported as a gap rather than as "no results". **99 work
...
patent. This can sweep ~100 of those and tells you which ones stayed silent.

The core sweep needs no API key. GitHub code search and Bluesky accept optional
credentials, Semantic Scholar benefits from a free key, and the Jev router is
optional (see [Optional credentials](#optional-credentials)); without them,
...
python3 scripts/deep_research.py "topic" --quick          # 52 keyless + optional Bluesky
...
| `academic` | 18 | Crossref, OpenAlex, Semantic Scholar, OpenReview, arXiv, PubMed, Europe PMC, Zenodo, DataCite, DOAJ, OpenAIRE, HAL, OSF, Figshare, Unpaywall, OpenCitations, HF Papers, + SearXNG scientific publications |
| `code` | 15 | GitHub, GitLab, Codeberg, deps.dev, npm, crates.io, Packagist, Maven, Docker Hub, HuggingFace, Software Heritage, SearXNG repos/IT/packages |
...
| `security` | 18 | CISA KEV, NVD, EPSS, CIRCL, OSV, GitHub Advisories, OpenSSF Scorecard, CertSpotter CT, Shodan InternetDB, RIPEstat, AlienVault OTX, OpenPhish, ExploitDB, Red Hat CVE, Ubuntu CVE, SigmaHQ, MITRE CWE, MITRE CAPEC |
...
queries activate deps.dev, GitHub Advisories, and Scorecard without spraying
unrelated package registries. MITRE's explicit CWE relationships resolve CAPEC
patterns for an explicit `CWE-<n>` (the two ID namespaces are never assumed to
match); domain APIs need a domain; package enrichers need an explicit
`ecosystem:package@version` shape. Misfits are reported as "not applicable"
instead of failing.

**Automatic routing is hybrid, not model-only.** `--auto` always runs a small
web discovery baseline. Exact CVE/GHSA/CWE, domain, URL, IP, ASN, LEI, hash,
DOI, arXiv, NCT, patent, repository and package shapes force their relevant
lanes, so Jev cannot veto them; a bare exact identifier is routed locally
without calling Jev. For ambiguous natural-language queries, Jev scores ten
...
Two sources require optional credentials, Semantic Scholar benefits from a free
key, and `--auto` can optionally use Jev. All degrade cleanly when unconfigured.
...
# Optional private 1-RPS quota; unauthenticated Semantic Scholar is supported
# but shares a heavily throttled public pool.
...
roughly 100 configured sources for nothing; use `--auto`, `--quick`, or plain search.
